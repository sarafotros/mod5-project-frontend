...
class Bookings extends React.Component {
			...
			this.setState({
				requests:
					localStorage.role === 'user'
						? requests.filter((req) => req.user_id == localStorage.user_id)
						: requests,
			})
		);
	};
	...
	removeBooking = (id) => {
		API.removeRequest(id).then((resp) => {
			if (resp.status) {
				this.getRequests();
			}
		});
	...
	editBooking = (id) => {
		const obj = {
			handy_man_id: parseInt(localStorage.handyman_id),
			'confirmed?': true,
		};
		API.editRequest(obj, id).then(() => {
			this.getRequests();
		});
	};
			...
			<>
				{requests.length > 0 ? (
					...
					<Paper className={classes.requestEmpty}>
						<Typography variant="h5">You have no request to show</Typography>
						<Button
							variant="contained"
							color="primary"
